...
from feed.market_feed.config import CONSUMER_KEY, MOBILE_NUMBER, UCC, MPIN, INSTRUMENT_TOKENS
...
class NeoFeedClient:
            ...
            try:
                ...
                if hasattr(self.client, "connect_websocket"):
                    try:
                        ...
                        print("[NeoFeedClient] WebSocket connect failed:", e)
                        return False
    ...
    def _run_forever(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # wait until a failure happens (on_close or on_error will set flags and loop will retry)
                # just sleep little bit to yield
                time.sleep(1)
    ...
    def _watchdog(self):
        while not self._stop_event.is_set():
            ...
            if age > self.watchdog_threshold:
                ...
                # Force restart: clear struct and let run loop re-do login/subscribe
                with self.lock:

                    try:
                        # Attempt to close any internal websockets if SDK exposes
                        if hasattr(self.client, "close_websocket"):
                            try:
                        ...
                        print("[NeoFeedClient][Watchdog] Error while forcing restart:", e)

                # give the run loop a moment
                time.sleep(1)

    def _on_open(self, *args, **kwargs):
        print("Neo SDK connection opened", args, kwargs)
    ...
    def _on_close(self, *args):
        ...
        # Some SDKs require explicit internal close to free resources
        try:
            if hasattr(self.client, "close_websocket"):
                self.client.close_websocket()
    ...
    def stop(self):
        ...
        # attempt to close SDK sockets (if SDK exposes)
        try:
            if hasattr(self.client, "close_websocket"):
                self.client.close_websocket()
